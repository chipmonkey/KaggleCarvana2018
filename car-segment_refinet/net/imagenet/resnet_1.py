...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, in_shape=(3,244,244), num_classes=1000):
        ...
        ## config ##
        self.config ='config-n2'

        if self.config =='original':  #-------------------------
            self.fc = nn.Linear(512 * block.expansion, num_classes)


        if self.config =='config-n1':  #----------------------
            self.layer5 = nn.Sequential(
                nn.Conv2d(512 * block.expansion, 64, kernel_size=3, stride=1, padding=1, bias=False),
                nn.BatchNorm2d(64),
                nn.ReLU(inplace=True),
        ...
        if self.config =='config-n2':  #----------------------
            ...
            self.short5 = nn.Sequential(
                nn.Conv2d(512 * block.expansion, 64, kernel_size=3, stride=1, padding=1, bias=True),
    ...
    def forward(self, x):
        ...
        if self.config =='config-n2':  #---------------------
            x = self.layer5(x) + self.short5(x)
            x = self.layer6(x) + self.short6(x)
            x = F.adaptive_avg_pool2d(x, output_size=1)
            x = x.view(x.size(0), -1)
            x = self.fc (x)
            prob = F.sigmoid(x)
